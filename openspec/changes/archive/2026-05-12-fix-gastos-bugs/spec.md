# Delta — fix-gastos-bugs

## Purpose

Corregir 11 bugs en el módulo de gastos que causan fuga de datos entre farmacias, falsos 403, permisos inconsistentes, y validaciones rotas. No se agregan nuevas capacidades.
...
The `startDate` and `endDate` query params MUST accept `YYYY-MM-DD` format. Using `z.string().datetime()` SHALL be replaced with `z.coerce.date()` or regex accepting ISO date strings.
...
- GIVEN an Expense document with a renamed custom function (e.g. `_isModified`)
